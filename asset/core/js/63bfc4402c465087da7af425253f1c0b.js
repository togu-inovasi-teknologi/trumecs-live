...
$("select[name=komponen]").change(function (argument) {
  ...
  $("select[name=sub_kategori]").load(
    ...
    function () {
      // Reset jenis kategori saat sub kategori berubah
      $("select[name=jenis_kategori]").html('<option value="">-- Semua Jenis --</option>');
      
      if (value && value != "0") {
        // Jika ada sub kategori yang dipilih sebelumnya, set nilainya
        if (
          ...
          // Load jenis kategori berdasarkan sub kategori yang dipilih
          var selectedSub = $(this).val();
          if (selectedSub && selectedSub != "0") {
            $("select[name=jenis_kategori]").load(
              base_url + "general/getjeniskategori/" + selectedSub,
              function() {
                if (
                  $("select[name=jenis_kategori] option[value='" +
                    $(".form-filter-search").attr("seletedjenis") +
                    "']").length > 0
                ) {
                  $(this).val($(".form-filter-search").attr("seletedjenis"));
                }
              }
            );
          }
  ...
  // Load merek berdasarkan komponen
  ...
  // Load quality berdasarkan komponen
...
// Event handler untuk perubahan sub kategori
$(document).on('change', 'select[name=sub_kategori]', function() {
  var value = $(this).val();
  ...
  if (value && value != "0") {
    $("select[name=jenis_kategori]").load(
      base_url + "general/getjeniskategori/" + value,
      function() {
        // Set nilai jenis kategori yang sudah dipilih sebelumnya
        if (
          $("select[name=jenis_kategori] option[value='" +
            $(".form-filter-search").attr("seletedjenis") +
            "']").length > 0
...
$(document).ready(function () {
  setTimeout(function () {
    ...
    var seletedjenis = formsearch.attr("seletedjenis");
    ...
    if (seletedcomponent != "") {
      ...
      $("select[name=sub_kategori]").load(
        ...
        function () {
          if (seletedsub && seletedsub != "") {
            if (
              $("select[name=sub_kategori] option[value='" + seletedsub + "']").length > 0
            ) {
              $(this).val(seletedsub);
              
              // Load jenis kategori berdasarkan sub kategori
              if (seletedsub != "0") {
                $("select[name=jenis_kategori]").load(
                  base_url + "general/getjeniskategori/" + seletedsub,
                  function() {
                    if (seletedjenis && seletedjenis != "") {
                      if (
                        $("select[name=jenis_kategori] option[value='" + seletedjenis + "']").length > 0
                      ) {
                        $(this).val(seletedjenis);
                      }
                    }
                  }
                );
      ...
      $("select[name=merek]").load(
        base_url + "general/getmerekall/" + seletedcomponent + "/false",
        function () {
          if (seletedbrand && seletedbrand != "") {
            if (
              $("select[name=merek] option[value='" + seletedbrand + "']").length > 0
            ) {
              $(this).val(seletedbrand);
            } else {
              $(this).val("");
            }
          }
      ...
      $("select[name=quality]").load(
        ...
        function () {
          if (seletedquality && seletedquality != "") {
            if (
              $("select[name=quality] option[value='" + seletedquality + "']").length > 0
            ) {
              $(this).val(seletedquality);
            } else {
              $(this).val("");
            }
          }
