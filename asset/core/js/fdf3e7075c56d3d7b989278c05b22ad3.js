...
$(document).ready(function () {
  ...
  $("#addFormSubCategoriExpert").on("submit", function (e) {
    e.preventDefault();

    $.ajax({
      url: base_url + "backendproduct/mainCategoriesExpertAjaxAdd",
      type: "POST",
      data: $(this).serialize(),
      dataType: "json",
      success: function (response) {
        if (response.status) {
          $("#add-subcategori-expert").modal("hide");
          $("#addFormSubCategoriExpert")[0].reset();
          tableCategoriExpert.ajax.reload();
          showSuccessToast(response.message);
        } else {
          showErrorToast(response.message);
        }
  ...
  function loadMainCategoriesExpert() {
    $.ajax({
      url: base_url + "backendproduct/getMainCategoriesExpert",
      type: "GET",
      dataType: "json",
      success: function (response) {
        if (response.status) {
          var select = $("#mainCategoriExpert");
          select.empty(); // Kosongkan dulu
          select.append('<option value="">Pilih Kategori Utama</option>');
          $.each(response.data, function (index, category) {
            select.append(
  ...
  // Panggil function ketika modal dibuka
  $("#add-subcategori-expert").on("show.bs.modal", function () {
    loadMainCategoriesExpert();
  });
