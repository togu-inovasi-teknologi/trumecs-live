...
// Load kategori utama
$("select[name=category]").load(base_url + "general/getcomponentall");
...
$("select[name=category]").change(function () {
  ...
  if (categoryId) {
    ...
    $("select[name=subCategory]").load(
      base_url + "general/getsubkategori/" + categoryId,
      function () {
        // Reset jenis kategori
        $("select[name=jenisCategory]").html(
          '<option value="">-- Semua Jenis --</option>'
        );

        // Jika ada subkategori yang dipilih sebelumnya
        var selectedSub = $(".form-filter-search").attr("seletedsub");
        if (
          selectedSub &&
          $("select[name=subCategory] option[value='" + selectedSub + "']")
            .length > 0
        ) {
          $(this).val(selectedSub);
          // Trigger change untuk load jenis kategori
          $(this).trigger("change");
        }
      }
    );
  } else {
    // Reset jika kategori utama dipilih "-- Semua kategori --"
...
// Ketika subkategori berubah
$("select[name=subCategory]").change(function () {
  var subCategoryId = $(this).val();

  if (subCategoryId) {
    // Load jenis kategori
    $("select[name=jenisCategory]").load(
      base_url + "general/getjeniskategori/" + subCategoryId,
      function () {
        // Jika ada jenis yang dipilih sebelumnya
        var selectedJenis = $(".form-filter-search").attr("selectedjenis");
        if (
          selectedJenis &&
          $("select[name=jenisCategory] option[value='" + selectedJenis + "']")
            .length > 0
        ) {
          $(this).val(selectedJenis);
        }
      }
    );
  } else {
    // Reset jika subkategori dipilih "-- Semua Subkategori --"
    $("select[name=jenisCategory]").html(
      '<option value="">-- Semua Jenis --</option>'
    );
  }
...
// Load merek berdasarkan kategori
$("select[name=category]").change(function () {
  var categoryId = $(this).val();

  if (categoryId) {
    $("select[name=merek]").load(
      base_url + "general/getmerekall/" + categoryId + "/false",
      function () {
        var selectedBrand = $(".form-filter-search").attr("seletedbrand");
        if (
          selectedBrand &&
          $("select[name=merek] option[value='" + selectedBrand + "']").length >
            0
        ) {
          $(this).val(selectedBrand);
        }
      }
    );

    $("select[name=quality]").load(
      base_url + "general/getgradeall/" + categoryId + "/false",
      function () {
        var selectedQuality = $(".form-filter-search").attr("seletedquality");
        if (
          selectedQuality &&
          $("select[name=quality] option[value='" + selectedQuality + "']")
            .length > 0
        ) {
          $(this).val(selectedQuality);
        }
      }
    );
  }
...
$(document).ready(function () {
  setTimeout(function () {
    ...
    var selectedComponent = formsearch.attr("seletedcategory");
    var selectedSub = formsearch.attr("seletedsub");
    var selectedJenis = formsearch.attr("selectedjenis");
    var selectedBrand = formsearch.attr("seletedbrand");
    var selectedQuality = formsearch.attr("seletedquality");

    // Set kategori utama jika ada
    if (selectedComponent) {
      $("select[name=category]").val(selectedComponent);

      // Load subkategori
      $("select[name=subCategory]").load(
        base_url + "general/getsubkategori/" + selectedComponent,
        function () {
          if (
            selectedSub &&
            $("select[name=subCategory] option[value='" + selectedSub + "']")
            ...
            // Load jenis kategori
            $("select[name=jenisCategory]").load(
              base_url + "general/getjeniskategori/" + selectedSub,
              function () {
                if (
                  selectedJenis &&
                  $(
                    "select[name=jenisCategory] option[value='" +
                      selectedJenis +
                      "']"
                  ).length > 0
                ) {
      ...
      // Load merek dan quality
      if (selectedBrand) {
        $("select[name=merek]").load(
          base_url + "general/getmerekall/" + selectedComponent + "/false",
          function () {
            if (
              selectedBrand &&
              $("select[name=merek] option[value='" + selectedBrand + "']")
                .length > 0
            ) {
              $(this).val(selectedBrand);
            }
      ...
      if (selectedQuality) {
        $("select[name=quality]").load(
          base_url + "general/getgradeall/" + selectedComponent + "/false",
          function () {
            if (
              selectedQuality &&
              $("select[name=quality] option[value='" + selectedQuality + "']")
