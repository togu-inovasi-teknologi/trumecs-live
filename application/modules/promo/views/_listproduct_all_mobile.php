        ...
        <?php foreach ($listproduct as $key) : ?>
            <div itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product" class="col-lg-3 col-md-4 col-sm-6 col-12 d-flex">
                <div class="card h-100 w-100 border-0 shadow-sm rounded-4 overflow-hidden position-relative">
                    <a itemprop="url" href="<?php echo base_url() ?>product/<?php echo $key["id"] ?>/<?php echo preg_replace("/[^a-zA-Z0-9]/", "-", ucwords(strtolower($key["tittle"]))) ?>" class="text-decoration-none click-product-promo-mobile" data-google-tag="<?php echo $key['id_promo'] ?> - <?php echo $key['tittle'] ?>">
                        <?php
                        $lfp = strlen($key["img"]);
                        ...
                        <div class="position-relative bg-light" style="height: 200px; overflow: hidden;">
                            <img src="<?php echo base_url() ?>timthumb?h=300&src=<?php echo base_url() ?>public/image/product/<?php echo ($ext == ".jpg") ? $key["img"] : "../noimage.png"; ?>"
    ...
    <?php } else { ?>
        <div class="col-12">
            <div class="alert alert-warning text-center border-0 rounded-4 py-4">
                ...
                <h5 class="mb-0">Pencarian tidak ditemukan</h5>
            </div>
        </div>
    <?php } ?>
</div>
