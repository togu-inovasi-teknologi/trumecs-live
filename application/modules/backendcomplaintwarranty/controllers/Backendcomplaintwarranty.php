...
class Backendcomplaintwarranty extends MX_Controller {
    ...
    function ambil_data(){
        ...
        $total=$this->db->count_all_results("complaintwarranty");
        $output=array();
        $output['draw']=$draw;
        ...
        $query=$this->db->get('complaintwarranty');
        if($search!=""){
            $this->db->where("(
                            idorder LIKE '%$search%' 
                            OR datecomplaint LIKE '%$search%'
                        )",'', false);
            $jum=$this->db->get('complaintwarranty');
            $output['recordsTotal']=$output['recordsFiltered']=$jum->num_rows();
        }

        foreach ($query->result_array() as $warranty) {
       
             $s = $warranty["status"]!="waiting respon" ? "success" : "danger";

            $output['data'][]=array(


            '<a class="fbold f14 forange" href="'.base_url().'backendcomplaintwarranty/detail/'.$warranty["id"].'">'.$warranty["idorder"].'</a><br>
                    <small><a class="fblack" href="'.base_url().'backendmember/detail/'.$warranty["idmember"].'">lihat pemesan</a></small>',

            '<span  class="fbold f14 black">'.$warranty["datecomplaint"].'</span><br><small class="fbold f12 black">'.$warranty["date"].'</small>',

            '<span  class="fbold f14 black label label-'.$s.'">'.$warranty["status"].'</span><br>'
                );
