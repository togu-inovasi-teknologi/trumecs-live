...
class etx_model extends CI_Model {
    ...
    {
        $this->db->where($where);
        $query1 = $this->db->get("complaintwarranty");
        $queryconfirm=$query1->result_array();
        $returnconfirm["complaintwarranty"] = $queryconfirm[0];
    ...
    {
        $this->db->where($where)->set($set)->update("complaintwarranty");
    }
    
     public function hapus($where)
    {
        $this->db->where($where)->delete("complaintwarranty");
    }
