<div class="galery">
	<div class="row">
		<div class="col-md-7">
			<strong class="f18">Galery Produk <?php echo $lelang["judul"] ?></strong>
		</div>
		<div class="col-md-5 <?php echo  (!$this->agent->is_mobile()) ? "text-right" : "" ; ?>">
			<form action="<?php echo base_url() ?>backendlelang/addgalery/" method="POST" enctype="multipart/form-data">
		      	<label class="file text-left">
					  <input type="file" id="file" name="filegalery" required >
					  <input type="hidden" name="id" value="<?php echo $lelang["id"] ?>" required>
					  <span class="file-custom">Pilih Gambar / </span>
				</label>
				<?php if (count($lelang["galery"])<=4): ?>
	...
	<div class="row">
		<div class="col-md-6">
			...
			<?php 
			$filename = 'public/image/lelang/'.$lelang["img"];
			if (file_exists($filename)) {
			    echo '<a href="#" class="pop"><img src="'.base_url().'public/image/lelang/'.$lelang["img"].'" class="img-fluid"></a>';
			} else {
