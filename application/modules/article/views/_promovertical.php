<?php $img_promo_red = '<img alt="promo trumecs" class="promo-small" src="' . base_url() . 'timthumb?w=70&src=' . base_url() . 'public/image/promo-special.png" width="70">'; ?>
<div class="heading_promo col-md-12 p-y-1">
	<strong><?php echo $promo_inseach_ver["promo"][0]["name"] ?></strong>
	<a class="btn btn-secondary" data-placement="left" data-toggle="popover" data-delay='{"show":"100", "hide":"10000"}' data-content="<div><?php echo $promo_inseach_ver["promo"][0]["description"] ?> <hr><a href='<?php echo base_url() . 'promo/' . $promo_inseach_ver["promo"][0]["url"] ?>' class='btn col-lg-12 btnnew'>Lihat Promo</a><br><br></div>" data-html="true" data-original-title="<?php echo $promo_inseach_ver["promo"][0]["name"] ?>" data-trigger="hover click"><i class="fa fa-tags"></i></a>
</div>
<div class="list_promo col-md-12">
	<?php foreach (array_slice($promo_inseach_ver["product"], 0, 6) as $key) : ?>
		<div itemprop="itemListElement" itemscope="" itemtype="http://schema.org/Product" class="col-lg-12 col-md-12 text-left hv_product ">
			<a itemprop="url" href="<?php echo base_url() ?>product/<?php echo $key["id"] ?>/<?php echo preg_replace("/[^a-zA-Z0-9]/", "-", ucwords(strtolower($key["tittle"]))) ?>">
				...
				<span class="infolablepromo"><?php echo $promo_inseach_ver["promo"][0]["name"] ?></span><br>
				...
				<span id="btnbuy<?php echo $key["id"] ?>" class="btn btnnew col-lg-12 vhidden"><i class="fa fa-shopping-cart"></i> Beli</span>
			</a>
		</div>
	<?php endforeach ?>
</div>
