    ...
    public function cost_delivery_ojek(){
        ...
        /*if($distance > $web_setting->row()->max_kirim_km){
            ...
            foreach($tarif->result() as $dt_tarif){
                ...
                $deliver_cost[] = array('kurir' => $dt_tarif->nama_kurir, 'tarif' => floor($cost),
                	'distance' =>$distance,'max_kilometer' => $dt_tarif->max_km,
                	'total_berat' => $total_berat,'max_berat' => $dt_tarif->max_berat,
                 	'id_tarif' => $dt_tarif->id_ojek_online , 'alamat' => $alamat,'detail_address'=>$this->input->post('detail_address') 
                 );
                //$no++;
            }
